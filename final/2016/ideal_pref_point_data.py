...
for idx in range(n):
	...
	for d_ in d:
		...
		box[idx][0] = prob.value

#Solve ideal max
for idx in range(n):
	obj = cp.Minimize(-c_ideal_max[idx])
	constraints = [c_ideal_max >= 0, c_ideal_max <= 1]
	for d_ in d:
		i = d_[0]
		j = d_[1]
		constraints += [(c[j] - c[i]).T @ c_ideal_max <= 0.5 * (c[i] + c[j])    .T @ (c[j] - c[i])]
		prob = cp.Problem(obj, constraints)
		prob.solve(solver = cp.CVXOPT)
		box[idx][1] = -prob.value
